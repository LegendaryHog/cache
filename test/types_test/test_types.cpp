...
std::vector<int> slow_get_page (int key) {return {key, 2*key, 3*key};}

int test (test_inf& t_inf, cache::cache_t<std::vector<int>>& my_cache);
...
int main(int argc, char* argv[])
{
    ...
    std::vector<cache::cache_t<std::vector<int>>> vec_my_cache (argc - 1);
    for (auto i = 0; i < (argc - 1)/2; i++)
    {
        cache::cache_t<std::vector<int>> new_cache (vec_t_inf[i].cache_size);
